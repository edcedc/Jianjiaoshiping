...
public class HistoricalPresenter extends HistoricalContract.Presenter{
    @Override
    public void onRequest(int pagerNumber) {
        CloudApi.list(pagerNumber, CloudApi.historyPage)
    ...
    public void onDel(List<String> listStr) {
        CloudApi.historyDeleteBatches(listStr)
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                    public void accept(Disposable disposable) throws Exception {
                        mView.showLoading();
